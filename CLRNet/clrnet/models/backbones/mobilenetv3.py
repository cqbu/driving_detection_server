...
class MobileNetV3Wrapper(nn.Module):
    def __init__(self, mobilenet='small', pretrained=True, cfg=None):
        super(MobileNetV3Wrapper, self).__init__()
        assert mobilenet in ['small', 'large']
        self.cfg = cfg
        self.model = eval('mobilenet_v3_' + mobilenet)(
...
class MobileNetV3(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(inverted_residual_setting)):
            cnf = inverted_residual_setting[i]
            layers.append(block(cnf, norm_layer))
            if cnf.stride == 2 and i != 0:
                self.output_layers.append(i)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        for i in range(len(self.features)):
            x = self.features[i](x)
            if i in self.output_layers or i == len(self.features) - 1:
                out.append(x)
